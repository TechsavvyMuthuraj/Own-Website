...
export default function Galaxy({
  ...
  useEffect(() => {
    ...
    try {
      ...
      console.warn("WebGL not supported for Galaxy component:", err);
      return;
    }
    ...
    function resize() {
      if (!ctn) return;
      // High-performance scaling: clamp DPR and render at 0.7x resolution, seamlessly upscaled via CSS
      const dpr = Math.min(typeof window !== "undefined" ? window.devicePixelRatio || 1 : 1, 1.25);
      const scale = dpr * 0.75;
      const width = Math.round((ctn.offsetWidth || window.innerWidth) * scale);
      const height = Math.round((ctn.offsetHeight || window.innerHeight) * scale);
